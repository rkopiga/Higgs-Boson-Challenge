...
def find_optimal_ws_Log_Reg_grouped(tX_grouped, y_grouped, initial_w_grouped, max_iters_grouped, gamma_grouped,
                                    regulator_grouped):
    optimal_ws = []
    for i in range(len(tX_grouped)):
        optimal_w, _ = logistic_regression_GD(y_grouped[i], tX_grouped[i], initial_w_grouped[i], max_iters_grouped[i],
