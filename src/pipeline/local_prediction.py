...
def local_prediction(tX, y,
    ...
    if group:
        ...
        for i in range(len(optimal_ws)):
            y_pred_grouped.append(predict_labels(optimal_ws[i], tX_sub_test_grouped[i]))
        compare_labels_grouped(y_pred_grouped, y_sub_test_grouped)
    else:
        tX_train, y_train, tX_sub_test, y_sub_test = separate_data(tX, y, ratio)
        optimal_w, _ = least_squares(tX_train, y_train)
        y_pred = predict_labels(optimal_w, tX_sub_test)
        compare_labels(y_pred, y_sub_test)
...
def compare_labels(y_pred, y_sub_test):
    accuracy = np.abs(y_pred + y_sub_test)
    unique, counts = np.unique(accuracy, return_counts=True)
    print("\tAccuracy : {}".format(counts[1] / len(accuracy)))


def compare_labels_grouped(y_pred_grouped, y_sub_test_grouped):
    for i in range(len(y_pred_grouped)):
        print("Group {}:".format(i))
        compare_labels(y_pred_grouped[i], y_sub_test_grouped[i])
